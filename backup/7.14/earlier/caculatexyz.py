...
def caculate_xyz(trajectory,projection, matrix,index):
    # fig = plt.figure()
    ...
    i = 0
    caculate_coordinates = np.empty((1,3))
    for i in range(0, len(projection)):
        ...
        if caculate_z is not None:

            append_coordinates = np.array([caculate_x,caculate_y,caculate_z])
            caculate_coordinates = np.vstack((caculate_coordinates,append_coordinates))
