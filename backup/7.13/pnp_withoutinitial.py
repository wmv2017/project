...
def project_slipview():
    ...
    '''
    read x,y,z from txt file
    '''
    # print(image_points.shape)
    validate_points = np.empty((1,3))
    in_file = open('points.txt','r')
    for line in in_file:
        line = line.strip().split(' ')
        line = list(map(float, line)) #change the string to number
        append_coordinates = np.array([line[1], line[2], line[3]])
        # np.append(validate_points, [[line[1], line[2], line[3]]], axis=0)
        validate_points = np.vstack((validate_points,append_coordinates))
    # print(validate_points)
    ...
    # print(translate_pts) 3d 121 1 2
    # translate_pts = np.vectorize(translate_pts)
...
'''
show the scatter and slip together
'''

def show_slip(slip_view):
    ...
    for i in range(1,slip_view.shape[0]):
        cv2.line(im, (int(former_x),int(former_y)), (int(slip_view[i][0]),int(slip_view[i][1])), (0,255,0), 1)
        former_x = slip_view[i][0]
        former_y = slip_view[i][1]
    cv2.imshow("Output", im)
    cv2.waitKey(0)
    # print(type(slip_view))
    # print(slip_view.shape)
